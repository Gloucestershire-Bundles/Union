  ...
  implements ICommandHandler<UpdateReferralStatusCommand>
{
  ...
  async execute(command: UpdateReferralStatusCommand): Promise<void> {
    ...
    this.logger.debug(
      `[${UpdateReferralStatusHandler.name}] Attempting to update referral ${reference} to status: ${newStatus}.`,
    );
    ...
    try {
      ...
      this.logger.warn(
        `[${UpdateReferralStatusHandler.name}] Invalid status transition for referral ${reference}: ${errorMessage}.`,
      );
    ...
    this.logger.log(
      `[${UpdateReferralStatusHandler.name}] Referral ${reference} status updated from ${oldStatus} to ${newStatus}.`,
    );
  }
}
